...
The skill picks an asset by matching the active theme's palette family + the brief's tone, then references it by absolute URL. Picking logic lives in Step 4 of [SKILL.md](../UPSTREAM_SKILL.md) — eyeball the manifest, choose the closest match, fall back if the file 404s.
